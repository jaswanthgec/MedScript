...
GOOGLE_API_KEY = "" #replace with ur keys
gemini_api_key = "" #replace with ur keys   
# Either set CHROME_DRIVER_PATH in your .env or use the following:
chrome_driver_path = os.getenv("CHROME_DRIVER_PATH", r"C:\Users\jaswanth\Downloads\chromedriver-win64\chromedriver-win64\chromedriver.exe")
...
# ================== FIRST PART: MEDICINE, CHAT & OTHER ROUTES ==================

# --- Medicine Price Scraping Function ---
...
def extract_text_from_image(image_file):
    ...
    try:
        ...
        # Filter for medical-related text
        medical_keywords = [
            "tablet", "capsule", "mg", "ml", "prescription", "dose", "medication", "medicine", "pharmacy", "drug",
...
def format_response(response_text, max_words=120):
    ...
    prescription_keywords = ["dosage", "frequency", "duration", "usage", "precautions"]
    ...
    for line in lines:
        ...
        if any(keyword in line_lower for keyword in prescription_keywords):
            extracting = True
        if extracting:
            prescription_section.append(line.strip())

    response_text = "\n".join(prescription_section).strip()
...
def medical_chatbot(prompt):
    ...
    identified_problems = identify_problem(prompt)
    if identified_problems:
        prompt = f"You are a helpful medical assistant. The user has the following problems: {', '.join(identified_problems)}. Please provide medication recommendations for each problem. Respond with a markdown format"

    prescription_pattern = re.compile(r'\b(syp|tab|cap|inj|cream|ointment|drop|spray)\s+([A-Za-z0-9\-]+)\s*\(([^)]+)\)\s*(\d+)\s*ML\s*(Q\d+H|TDS|SOS|TOS)\s*x\s*(\d+d)\b', re.IGNORECASE)
    if prescription_pattern.search(prompt):
        prompt = f"You are a medical assistant. The user provided the following prescription instructions: {prompt}. Please explain the usage and precautions for each medication."
...
# --- Function: Get Predicted Disease based on Symptoms ---
def get_predicted_value(patient_symptoms):
    input_vector = np.zeros(len(symptoms_dict))
    logging.debug("Initial input vector: %s", input_vector)
    ...
    for item in patient_symptoms:
        ...
        if index != -1:
            ...
            logging.debug("Symptom '%s' mapped directly to index %s", item, index)
        else:
            found = False
            for key, synonyms in symptom_mapping.items():
                if item in synonyms:
                    ...
                    if index != -1:
                        input_vector[index] = 1
                        logging.debug("Symptom '%s' mapped to synonym '%s' with index %s", item, key, index)
                        found = True
                        break
            if not found:
                ...
                if index != -1:
                    input_vector[index] = 1
                    logging.debug("Symptom '%s' corrected to '%s' and mapped to index %s", item, corrected_item, index)
                else:
                    found_inner = False
                    for key, synonyms in symptom_mapping.items():
                        if corrected_item in synonyms:
                            ...
                            if index != -1:
                                input_vector[index] = 1
                                logging.debug("Symptom '%s' corrected to '%s' and mapped to synonym '%s' with index %s", item, corrected_item, key, index)
                                found_inner = True
                                break
                    if not found_inner:
                        logging.warning("Symptom '%s' with corrected spelling '%s' not found in mapping", item, corrected_item)
